...
def render_video_dna_storage_panel() -> None:
    st.markdown("## Video DNA Storage")
    st.caption("Fault-tolerant AV compressor: wavelet visual payload + optional μ-law audio payload, then shared DNA/strand pipeline.")
    _render_video_stepper()
    ...
    with st.container(border=True):
        ...
        if uploaded is None:
            ...
            c1.metric("Input file", uploaded.name)
            ...
            try:
                ...
                if info:
                    c3.metric("Resolution", f"{info.get('width', 0)}×{info.get('height', 0)}")
                    dur = info.get("duration")
                    c4.metric("Duration", f"{float(dur):.2f}s" if dur else "—")
                    f1, f2 = st.columns(2)
                    f1.metric("FPS", f"{float(info.get('fps') or 0):.2f}")
                    f2.metric("Audio track", "Yes" if has_audio else "No")
            except Exception as exc:
                st.warning(f"Could not read video metadata. Check FFmpeg/ffprobe. Details: {exc}")

    # 2. Video Compression
    with st.container(border=True):
        _step_header(2, "Video Compression")
        if not st.session_state.get("video_input_path"):
            ...
            if st.button("Run Video Compression", key="run_video_compression", type="primary"):
                try:
    ...
    with st.container(border=True):
        ...
        if not payload:
            ...
            mapping = st.selectbox("DNA method", options, format_func=_display_mapping, key="video_mapping_select")
            ...
            if dna:
                ...
                m1, m2, m3, m4 = st.columns(4)
                m1.metric("DNA method", _display_mapping(st.session_state.get("video_mapping", mapping)))
                ...
                with st.container(border=True):
                    st.markdown("#### Quick substitution test — skip Strand Design")
                    q1, q2 = st.columns(2)
                    ...
                    st.caption("Fast substitution-only test on the full encoded DNA. This bypasses Strand Design and is recommended for multi-million-nt video DNA.")
    ...
    # 5. Video Reconstruction
    with st.container(border=True):
        _step_header(5, "Video Reconstruction")
        if not st.session_state.get("video_dna"):
            st.info("Run DNA Encoding first.")
        else:
            uploaded_strands = st.file_uploader("Upload strands CSV for reconstruction", type=["csv"], key="video_decode_upload_strands_csv")
            ...
            if st.button("Run Video Reconstruction", key="run_video_reconstruction", type="primary", disabled=not bool(selected_input_dna)):
    ...
    with st.container(border=True):
        ...
        if not st.session_state.get("video_reconstructed_result") and not st.session_state.get("video_decode_error"):
            ...
            payload = st.session_state.get("video_payload", b"")
            uploaded_size = len(st.session_state.get("video_input_bytes", b""))
            raw_rgb = int(manifest.get("raw_rgb_bytes", 0))
            rows = [
                {"Metric": "Video mode", "Value": st.session_state.get("video_mode_select", "-")},
                {"Metric": "Payload bytes", "Value": _fmt_bytes(len(payload))},
                {"Metric": "Compression vs uploaded file", "Value": f"{uploaded_size / max(1, len(payload)):.2f}x"},
                {"Metric": "Compression vs raw RGB", "Value": f"{raw_rgb / max(1, len(payload)):.2f}x"},
                ...
                {"Metric": "Temporal repair window", "Value": f"{float((manifest.get('config', {}) or {}).get('temporal_repair_window_sec', 0.0)):.2f}s"},
                {"Metric": "Repair radius", "Value": f"±{int((manifest.get('config', {}) or {}).get('repair_radius', 0))} frames"},
                {"Metric": "Frame repair", "Value": (manifest.get('config', {}) or {}).get('repair_strength', '-')},
                {"Metric": "Protection mode", "Value": (manifest.get('protection', {}) or {}).get('mode', 'None')},
                {"Metric": "Protected frames", "Value": f"{int((manifest.get('protection', {}) or {}).get('protected_frames', 0)):,}"},
                {"Metric": "Protection overhead", "Value": _fmt_bytes(int((manifest.get('protection', {}) or {}).get('payload_bytes', 0)))},
                {"Metric": "Frame count", "Value": f"{int(visual.get('frame_count', 0)):,}"},
                {"Metric": "Reconstruction source", "Value": metrics.get("reconstruction_source", "-")},
                {"Metric": "Exact bytes", "Value": "Yes" if metrics.get("byte_exact") else "No"},
                {"Metric": "Byte error rate", "Value": f"{float(metrics.get('byte_error_rate', 0.0)) * 100:.2f}%"},
                {"Metric": "Playable", "Value": "Yes" if metrics.get("playable") else "No"},
                {"Metric": "Base PSNR before repair", "Value": f"{float(metrics.get('base_video_psnr', 0.0)):.2f} dB"},
                {"Metric": "Video PSNR", "Value": f"{float(metrics.get('video_psnr', 0.0)):.2f} dB"},
                {"Metric": "PSNR gain from repair", "Value": f"{float(metrics.get('psnr_gain_from_repair', 0.0)):+.2f} dB"},
                {"Metric": "Base SSIM before repair", "Value": f"{float(metrics.get('base_video_ssim', 0.0)):.4f}"},
                {"Metric": "Video SSIM", "Value": f"{float(metrics.get('video_ssim', 0.0)):.4f}"},
                {"Metric": "SSIM gain from repair", "Value": f"{float(metrics.get('ssim_gain_from_repair', 0.0)):+.4f}"},
                {"Metric": "Frame correlation", "Value": f"{float(metrics.get('frame_correlation', 0.0)):.4f}"},
                {"Metric": "Frames compared", "Value": f"{int(metrics.get('frames_compared', 0)):,}"},
                {"Metric": "Audio SNR raw", "Value": f"{float(metrics.get('audio_snr_raw_db', 0.0)):.2f} dB" if audio.get("enabled") else "—"},
                {"Metric": "Audio SNR cleaned", "Value": f"{float(metrics.get('audio_snr_clean_db', 0.0)):.2f} dB" if audio.get("enabled") else "—"},
                {"Metric": "Reconstruction time", "Value": f"{float(metrics.get('reconstruction_time_seconds', 0.0)):.2f}s"},
            ]
