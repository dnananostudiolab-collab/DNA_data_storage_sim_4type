...
def render_audio_dna_storage_panel() -> None:
    st.markdown("## Audio DNA Storage")
    _render_audio_stepper()
    ...
    # 2. Audio Compression
    with st.container(border=True):
        _step_header(2, "Audio Compression")
        ...
        if not raw:
            ...
            if st.button("Run Audio Compression", key="run_audio_compression", type="primary"):
                try:
    ...
    with st.container(border=True):
        ...
        if not payload:
            ...
            options = [m for m in MAPPING_OPTIONS if m in {"Simple Mapping", "RINF_B16", "R0_B9", "R1_B12", "R2_B15", "New Design"}]
            if not options:
                options = ["Simple Mapping"]
            mapping = st.selectbox("DNA method", options, format_func=_display_mapping, key="audio_mapping_select")
            ...
            if dna:
                ...
                m1.metric("DNA method", _display_mapping(st.session_state.get("audio_mapping", mapping)))
                ...
                with st.container(border=True):
                    ...
                    st.caption("Fast substitution-only test on the full encoded DNA. This bypasses Strand Design and is useful for long audio DNA.")
    ...
    # 5. Audio Reconstruction
    with st.container(border=True):
        _step_header(5, "Audio Reconstruction")
        noisy_dna = st.session_state.get("audio_noisy_dna")
        ...
        if st.session_state.get("audio_dna"):
            uploaded_strands = st.file_uploader(
                "Upload strands CSV for reconstruction",
                type=["csv"],
        ...
        if st.session_state.get("audio_dna") and (st.session_state.get("audio_noisy_dna") or st.session_state.get("audio_uploaded_decode_dna") or st.session_state.get("audio_dna")):
            if st.button("Run Audio Reconstruction", key="run_audio_reconstruction", type="primary", disabled=not bool(selected_input_dna)):
                if selected_decode_source == "Uploaded strands CSV":
    ...
    with st.container(border=True):
        ...
        if not st.session_state.get("audio_recovered_wav") and not st.session_state.get("audio_decode_error"):
            ...
            rows = [
                ...
                {"Metric": "Audio duration", "Value": f"{float(payload_meta.get('duration_seconds', 0.0)):.2f}s"},
                {"Metric": "Payload bytes", "Value": _fmt_bytes(len(st.session_state.get("audio_payload", b'')))},
                {"Metric": "Compression vs uploaded file", "Value": f"{float(payload_meta.get('uploaded_file_bytes', 0)) / max(1, len(st.session_state.get('audio_payload', b''))):.2f}x"},
                {"Metric": "Compression vs PCM16", "Value": f"{(int(payload_meta.get('sample_count', 0)) * 2) / max(1, len(st.session_state.get('audio_payload', b''))):.2f}x"},
                ...
                {"Metric": "PSNR", "Value": f"{float(metrics.get('psnr_db', 0.0)):.2f} dB"},
                ...
                {"Metric": "Waveform MSE", "Value": f"{float(metrics.get('waveform_mse', 0.0)):.6f}"},
                {"Metric": "Reconstruction time", "Value": f"{float(metrics.get('reconstruction_time_seconds', 0.0)):.2f}s"},
                {"Metric": "AI cleanup", "Value": metrics.get("ai_cleanup", "")},
            ]
