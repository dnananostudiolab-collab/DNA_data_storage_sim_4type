...
from ui_helpers import fmt_bytes
...
def _clean_decode_source_label(label: str) -> str:
    if label.startswith("Clean DNA"):
        return "Original encoded data"
    if label.startswith("Error DNA"):
...
def render_text_dna_storage_panel() -> None:
    ...
    with st.container(border=True):
        st.markdown("### 2. Text Compression")
        ...
        if st.button("Run method", key="text_run_selected_method", disabled=not bool(text.strip())):
            try:
    ...
    with st.container(border=True):
        st.markdown("### 3. DNA Encoding Mapping")
        package = st.session_state.get("text_selected_package")
        if not isinstance(package, dict):
            ...
            mapping_labels = [TEXT_DNA_MAPPING_LABELS[m] for m in TEXT_DNA_MAPPING_OPTIONS]
            mapping_label = st.radio(
                "DNA mapping",
                mapping_labels,
                horizontal=True,
                index=TEXT_DNA_MAPPING_OPTIONS.index(current_mapping),
                key="text_dna_mapping_label",
            )
            mapping_name = TEXT_DNA_MAPPING_OPTIONS[mapping_labels.index(mapping_label)]

            if st.button("Run DNA mapping", key="text_apply_mapping"):
                try:
    ...
    with st.container(border=True):
        ...
        if not dna:
            ...
            if isinstance(rows, list) and rows:
                ...
                enable_errors = st.checkbox("Add DNA errors", value=bool(st.session_state.get("text_advanced_error_rows")), key="text_enable_strand_level_errors")
                if enable_errors:
                    with st.container(border=True):
    ...
    with st.container(border=True):
        ...
        if not isinstance(package, dict) or not package.get("mapped_dna"):
            ...
            source_options = ["Clean DNA (before error)", "Error DNA (after simulation)", "Uploaded strands CSV"]
            ...
            if st.button("Decode", key="text_decode_button", disabled=not bool(decode_dna)):
                try:
                    ...
                    sub_df = _text_error_events_table(st.session_state.get("text_advanced_error_rows", [])) if source_label.startswith("Error DNA") else pd.DataFrame()
                    ...
                    st.session_state["text_validation"] = {
                        **metrics,
                        ...
                        "decode_language": decode_language,
                        "language_scope": language_scope,
                        "dna_error_substitutions": int(dna_error_counts.get("substitution", 0)) if source_label.startswith("Error DNA") else 0,
                        "dna_error_deletions": int(dna_error_counts.get("deletion", 0)) if source_label.startswith("Error DNA") else 0,
                        "dna_error_insertions": int(dna_error_counts.get("insertion", 0)) if source_label.startswith("Error DNA") else 0,
                        "dna_error_total": int(sum(dna_error_counts.values())) if source_label.startswith("Error DNA") else 0,
                        "substituted_bases": int(dna_error_counts.get("substitution", 0)) if source_label.startswith("Error DNA") else 0,
            ...
            if isinstance(validation, dict) and validation:
                ...
                r1.metric("Source", validation.get("decode_source", "—"))
                r2.metric("DNA substitutions", validation.get("dna_error_substitutions", 0))
                r3.metric("Language", DECODE_LANGUAGE_LABELS.get(validation.get("decode_language", "auto"), "Auto"))
                r4.metric("Invalid codes", validation.get("invalid_code_count", "—"))
                ...
                if isinstance(recovery_df, pd.DataFrame) and not recovery_df.empty:
                    ...
                    if not display_recovery.empty:
                        with st.expander("Token recovery preview", expanded=False):
                            st.dataframe(display_recovery.head(120), use_container_width=True, hide_index=True)

    with st.container(border=True):
        st.markdown("### 6. Validation")
        package = st.session_state.get("text_selected_package")
        validation = st.session_state.get("text_validation")
        final_text = st.session_state.get("text_final_text", "")
        decoded_text = st.session_state.get("text_decoded_text", "")
        ...
        if not isinstance(package, dict) or not isinstance(validation, dict) or not validation:
            ...
            with c1:
                ...
                st.text_area("Original", package.get("original_text", ""), height=280, key="text_original_compare")
            with c2:
                st.markdown("#### Decoded text" + (" / repaired" if repaired_text is not None else ""))
                recovery_df = st.session_state.get("text_token_recovery_df", pd.DataFrame())
                st.markdown(_annotated_decoded_html(recovery_df, final_text), unsafe_allow_html=True)

            m1, m2, m3, m4 = st.columns(4)
            m1.metric("Word accuracy", f"{validation['word_accuracy']*100:.2f}%")
            m2.metric("Word error rate", f"{validation['word_error_rate']*100:.2f}%")
            m3.metric("Exact word matches", f"{validation['exact_word_matches']:,}/{validation['original_words']:,}")
            m4.metric("Sequence similarity", f"{validation['final_similarity']:.4f}")

            d1, d2, d3 = st.columns(3)
            d1.metric("DNA substitutions", validation.get("dna_error_substitutions", 0))
            d2.metric("DNA deletions", validation.get("dna_error_deletions", 0))
            d3.metric("DNA insertions", validation.get("dna_error_insertions", 0))

            e1, e2, e3, e4 = st.columns(4)
            e1.metric("Word substitutions", validation["substitutions"])
            e2.metric("Word deletions", validation["deletions"])
            e3.metric("Word insertions", validation["insertions"])
            e4.metric("Token repair", "On" if validation.get("used_xlmr_repair") else "Off")
            ...
            recovery_df = st.session_state.get("text_token_recovery_df", pd.DataFrame())
            if isinstance(recovery_df, pd.DataFrame) and not recovery_df.empty:
                display_recovery = recovery_df[recovery_df["status"] != "Decoded"] if "status" in recovery_df.columns else recovery_df
                if not display_recovery.empty:
                    with st.expander("Token recovery preview", expanded=False):
                        st.dataframe(display_recovery.head(200), use_container_width=True, hide_index=True)

            # Detailed validation outputs are hidden in the compact UI.
